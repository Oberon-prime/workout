...
// === Константы АЦП ===
const float ADC_SCALE = (5.0f / 1023.0f) * 10.1f; // Объединили ADC_GAIN * DIVIDER
...
enum CurrentIndex {
  LOW_CURRENT = 0,   // >6 Ом
  MED_CURRENT = 1,   // 1.8-6 Ом  
  HIGH_CURRENT = 2   // <1.8 Ом
};
...
void onZC() {
  Timer1.setPeriod(delayTicks * 10); // delayTicks в единицах 10мкс
...
void startWelding() {
  ...
  // Получаем индекс тока (в реальности - из штрих-кода)
  float R_nom = 8.2f; // Пример сопротивления муфты
  ...
  while (millis() - startTime < WELD_TIME_MS) {
    ...
    if (now - lastCorrection >= CORRECTION_INTERVAL) {
      ...
      // Коррекция: Uavg -> Urms с учетом фазового управления
      float Urms_est = calcUrmsFromUavg(Uavg, (uint16_t)delayTicks, currentIdx);
      lastVoltage = Uavg; // Для отладки
